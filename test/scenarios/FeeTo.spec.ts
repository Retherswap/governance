...
import RetherswapFactory from '@retherswap/build/RetherswapFactory.json'
import RetherswapPair from '@retherswap/build/RetherswapPair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy retherswap', async () => {
    factory = await deployContract(wallet, RetherswapFactory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, Rethers, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, Rethers, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, RetherswapPair.abi).connect(wallet)
