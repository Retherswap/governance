...
describe('scenario:setFeeTo', () => {
  ...
  beforeEach('deploy retherswap', async () => {
    factory = await deployContract(wallet, RetherswapFactory, [timelock.address])
  })
  ...
  it('setFeeTo', async () => {
    ...
    const description = 'Set feeTo on the RetherswapFactory to the timelock address.'

    // activate balances
    await rethers.delegate(wallet.address)
