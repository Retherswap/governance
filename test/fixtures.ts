...
export async function governanceFixture(
  ...
  // deploy Rethers, sending the total supply to the deployer
  const { timestamp: now } = await provider.getBlock('latest')
  const timelockAddress = Contract.getContractAddress({ from: wallet.address, nonce: 1 })
  const rethers = await deployContract(wallet, Rethers, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, rethers.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { rethers, timelock, governorAlpha }
}
